...
for(const path of ['/',...tools.map(t=>'/'+t.slug),'/about','/contact','/privacy']){
 ...
 const html=await response.text();assert(html.includes('rel="canonical" href="'+SITE_ORIGIN+path+'"'),path);
...
const sitemap=await (await fetch(origin+'/sitemap.xml')).text();assert.equal((sitemap.match(/<loc>/g)||[]).length,(tools.length+4)*4);
const robots=await (await fetch(origin+'/robots.txt')).text();assert(robots.includes('Sitemap: '+SITE_ORIGIN+'/sitemap.xml'));
