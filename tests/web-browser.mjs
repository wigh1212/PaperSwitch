...
try{
 ...
 assert.equal(await page.locator('.conversion-link').count(),tools.filter(t=>t.type==='convert').length);
 ...
 for(const tool of tools){
  ...
  assert.equal(await page.locator('h1').textContent(),tool.title);
  assert.equal(await page.locator('meta[name="description"]').getAttribute('content'),tool.description);
 ...
 await raw.goto('http://127.0.0.1:4173/pdf-to-txt');assert.equal(await raw.locator('h1').textContent(),'PDF to TXT');assert(await raw.locator('.guide').first().isVisible());await noJS.close();
